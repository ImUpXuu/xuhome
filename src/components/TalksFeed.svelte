<script lang="ts">
  ...
  let foldTimers: ReturnType<typeof setTimeout>[] = [];

  function setupTalkFold() {
    // 清理上一轮留下的孤儿"收起"按钮（已脱离对应内容的）
    document.querySelectorAll('.talk-collapse-wrap').forEach(wrap => {
      const prev = wrap.previousElementSibling;
      if (!prev || !prev.classList.contains('talk-content') || prev.dataset.foldExpanded !== 'true') {
        wrap.remove();
      }
    });

    document.querySelectorAll('.talk-content').forEach(el => {
      // 已展开的内容不再重跑折叠逻辑（避免点"加载更多"/切标签后被重新折起）
      if (el.dataset.foldExpanded === 'true') return;
      // 已有收起按钮的也跳过
      if (el.nextElementSibling && el.nextElementSibling.classList.contains('talk-collapse-wrap')) return;
      ...
      btn.addEventListener('click', function expand() {
        ...
        const t = setTimeout(() => {
          ...
          collapseBtn.addEventListener('click', () => {
            delete el.dataset.foldExpanded;
            el.style.maxHeight = `${threshold}px`;

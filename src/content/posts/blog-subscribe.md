...
title: "基于 GitHub Issues 的博客更新订阅方?"
published: 2026-05-10 00:00:00
image: "https://img.476543.xyz/img/2026/5/10/1778388844407_935.png"
...
RSS？邮件推送？第三方通知服务？这些方案各有问题。今天分享一个我?GitHub Issues 实现的零成本、零配置的订阅方案?
...
很多 RSS 转邮件服务（Feedly、Inoreader 等）发出的通知邮件经常?Gmail、QQ 邮箱直接扔进垃圾箱。读者根本看不到更新通知?
...
自建 RSS 转邮件需要部署服务、配?SMTP、处理退订逻辑……对个人博客来说太重了。第三方服务又往往收费或者有使用限制?
...
2. ?Issue 页面右上角点?**Subscribe**（或评论任意内容自动订阅?

![subscribe](https://img.476543.xyz/img/2026/5/10/20260510092802_496.png)

就这么简单，不需要注册任何额外账号，?GitHub 账号就行?
...
整个方案只依赖一?GitHub Actions 工作流，核心逻辑如下?

1. ?`main` 分支?`push` 且变更了 `src/content/posts/**/*.md` 文件时触?
2. 获取变更的文件列?
3. ?Node.js 解析 Markdown ?frontmatter，提?**标题**?*描述**?*发布时间**
...
- **零成?*：完全免费，?GitHub 自带功能
- **不会被当垃圾邮件**：GitHub 的邮件可信度远高于第三方 RSS 转邮件服?
- **零配?*：博主不用管，读者评论一下就?
- **可追?*：所有更新历史都?Issue 的评论里，一目了?

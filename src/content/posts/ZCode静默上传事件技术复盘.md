...
keywords: ["ZCode", "智谱", "ZCode静默上传", "AI编程工具", "数据安全", "Git历史", "隐私合规", "逆向分析", "阿里云OSS"]
description: 2026 年 9 月 18 日智谱 ZCode 被曝登录态下静默打包工作区并上传完整 .git 历史。本文还原上传链路、快照构成与过滤逻辑，核对官方回应与流传说法，并给出自查与阻断方案。
...
我没法判断这是疏忽还是选择，报告里也没做这个判断。但这个组合确实很难用"手滑"解释。

另一件让我在意的事是"修复"的形态。默认值翻转 + 服务端下线，行为确实停了。但上传管线的代码还在客户端里，随时可以经服务端配置重新启用。这意味着现在安全是因为**服务端配置正确**，而不是因为**客户端做不到**。这两者的区别，在信任问题上不是小事。

还有一点关于舆论本身。这次事件在开发者社区炸得很响，HN 也上了首页，但头部 AI 科技媒体基本没跟进。这个落差我觉得挺有意思——可能因为涉事的是国产头部厂商，也可能因为大家默认"AI 工具读代码"是常态，读到什么程度才算越界反而没共识。

最后说点实际的。我自己也装过 ZCode，GLM-5.3 配它的体验确实不错，HCI 做得比不少同类工具顺手。所以这件事对我而言不是"要不要抵制"，而是一次很具体的提醒：**便宜和好用是真的，数据边界的问题也是真的，这两件事可以同时成立。**HN 评论区有位老哥说得挺实在——"ZAI 会遭到反弹吗？不会。人们会继续用。见鬼，我也会继续用。"

我不会因为这个就不用国产工具，但我会开始做几件以前懒得做的事：给 AI 工具单独建个用户、把敏感仓库用 shallow clone 隔离、把 `.env` 换成 sops 加密只在运行时注入。这些动作的成本不高，但能把"信任"从口头承诺变成自己手里的一道闸。

至于智谱承诺的开源和第三方审计，我会等。开源代码库是不是包含上传管线模块、审计机构是谁、结论会不会公开——这三件事比一份致歉声明有说服力得多。

> 本文技术细节来自公开的第三方逆向分析与独立复现，官方仅确认"Repo Wiki 生成时触发仓库数据上传、功能默认开启、已修复"。文中未发现任何证据表明上传数据被用于模型训练或被泄露；关于动机的讨论已明确标注为推测。截至本文发布，未见监管机构公开立案或表态。
